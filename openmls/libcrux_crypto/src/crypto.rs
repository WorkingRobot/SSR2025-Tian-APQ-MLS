...
impl OpenMlsCrypto for CryptoProvider {
    ...
    fn hkdf_extract(
        ...
        Ok(out.into()) // Convert Vec<u8> into SecretVLBytes
    }
...
fn hpke_kem(kem: HpkeKemType) -> hpke_rs_crypto::types::KemAlgorithm {
    match kem {
        ...
        | HpkeKemType::MlKem1024 => {
            unimplemented!("ML-KEM is not supported by the libcrux provider.")
        }
    }
}
...
// impl<'a, Rng: RngCore> Deref for GuardedRng<'a, Rng> {
//     type Target = Rng;
...
// impl<'a, Rng: RngCore> DerefMut for GuardedRng<'a, Rng> {
