...
impl From<hpke_rs_crypto::error::Error> for HpkeError {
    fn from(e: hpke_rs_crypto::error::Error) -> Self {
        match e {
            ...
            hpke_rs_crypto::error::Error::SliceError => {
                HpkeError::CryptoError("ML-KEM Key Derivation data conversion failed (TryFromSliceError)".to_string())
            }
            hpke_rs_crypto::error::Error::HpkeDecryptionError => {
                HpkeError::CryptoError("Ciphertext data conversion failed (likely due to broken ciphertext)".to_string())
            }
        }
    }
}
